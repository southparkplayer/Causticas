...
/**
 * Compute pass that decodes an ST.2084/PQ [0,1] image back to scRGB-linear — used on DLSS Frame Generation's
 * generated-frame output (which DLSS-FG writes in the PQ encoding {@link RtPqEncodePipeline} fed it as input,
 * since PQ is the only HDR encoding it supports) so the generated frame can be blitted into our scRGB
 * swapchain the same way a real frame is (see {@code shaders/rt/pq_to_hdr.comp}). Same descriptor shape as
 * {@link RtPqEncodePipeline}, just the inverse transform.
 */
public final class RtPqDecodePipeline {
    ...
    private RtPqDecodePipeline(RtContext ctx, long dsl, long pool, long set, long layout, long pipeline) {
        this.ctx = ctx;
    ...
    public static RtPqDecodePipeline create(RtContext ctx) {
        VkDevice vk = ctx.vk();
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            check(VK10.vkCreateDescriptorSetLayout(vk, dslci, null, p), "vkCreateDescriptorSetLayout(pq decode)");
            long dsl = p.get(0);
            RtDebugLabels.name(ctx, VK10.VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT, dsl, "pq decode descriptor set layout");
            ...
            check(VK10.vkCreateDescriptorPool(vk, dpci, null, p), "vkCreateDescriptorPool(pq decode)");
            long pool = p.get(0);
            RtDebugLabels.name(ctx, VK10.VK_OBJECT_TYPE_DESCRIPTOR_POOL, pool, "pq decode descriptor pool");

            VkDescriptorSetAllocateInfo dsai = VkDescriptorSetAllocateInfo.calloc(stack).sType$Default()
                    .descriptorPool(pool).pSetLayouts(stack.longs(dsl));
            LongBuffer pSet = stack.mallocLong(1);
            check(VK10.vkAllocateDescriptorSets(vk, dsai, pSet), "vkAllocateDescriptorSets(pq decode)");
            long set = pSet.get(0);
            RtDebugLabels.name(ctx, VK10.VK_OBJECT_TYPE_DESCRIPTOR_SET, set, "pq decode descriptor set");

            VkPipelineLayoutCreateInfo plci = VkPipelineLayoutCreateInfo.calloc(stack).sType$Default()
                    .pSetLayouts(stack.longs(dsl));
            check(VK10.vkCreatePipelineLayout(vk, plci, null, p), "vkCreatePipelineLayout(pq decode)");
            long layout = p.get(0);
            RtDebugLabels.name(ctx, VK10.VK_OBJECT_TYPE_PIPELINE_LAYOUT, layout, "pq decode pipeline layout");

            long module = loadModule(vk, stack, "pq_to_hdr.comp.spv");
            RtDebugLabels.name(ctx, VK10.VK_OBJECT_TYPE_SHADER_MODULE, module, "pq decode shader module");
            ...
            check(VK10.vkCreateComputePipelines(vk, VK10.VK_NULL_HANDLE, cpci, null, pPipeline),
                    "vkCreateComputePipelines(pq decode)");
            RtDebugLabels.name(ctx, VK10.VK_OBJECT_TYPE_PIPELINE, pPipeline.get(0), "pq decode compute pipeline");
            VK10.vkDestroyShaderModule(vk, module, null);

            return new RtPqDecodePipeline(ctx, dsl, pool, set, layout, pPipeline.get(0));
        }
    }

    /** Bind the scRGB destination (storage) and the PQ source (combined image sampler, GENERAL layout). */
    ...
    public void dispatch(VkCommandBuffer cmd, int width, int height) {
        try (MemoryStack stack = MemoryStack.stackPush(); RtDebugLabels.Scope ignored = RtDebugLabels.scope(ctx, cmd, "pq decode")) {
    ...
    private static long loadModule(VkDevice vk, MemoryStack stack, String name) {
        byte[] bytes;
        try (InputStream in = RtPqDecodePipeline.class.getResourceAsStream(SHADER_DIR + name)) {
            if (in == null) {
